...
def incroci_semantici(testo: str) -> Tuple[List[str], Dict[str, List[Tuple[int, int, List[int]]]]]:
    """
...
def crea_matrice_html(testo: str, indici_saltatoria: List[int], indici_semantici: List[int], larghezza_matrice: int = 50) -> str:
    """
    Crea una visualizzazione a matrice in formato HTML, con il testo distribuito in una griglia.
    
    Args:
        testo: Il testo da visualizzare
        indici_saltatoria: Indici dei caratteri da evidenziare per estrazione saltatoria
        indici_semantici: Indici dei caratteri da evidenziare per incroci semantici
        larghezza_matrice: Numero di colonne della matrice
        
    Returns:
        HTML della matrice visualizzata a griglia
    """
    testo_pulito = re.sub(r'\s+', '', testo).lower()
    
    # Calcola il numero di righe necessarie
    num_righe = math.ceil(len(testo_pulito) / larghezza_matrice)
    
    html_matrice = '<div class="matrice-container">\n'
    html_matrice += '<table class="matrice">\n'
    
    # Aggiungi intestazione numerica delle colonne
    html_matrice += '<tr><th></th>'
    for i in range(larghezza_matrice):
        html_matrice += f'<th>{i+1}</th>'
    html_matrice += '</tr>\n'
    
    # Aggiungi righe con caratteri
    for riga in range(num_righe):
        html_matrice += f'<tr><th>{riga+1}</th>'
        
        for colonna in range(larghezza_matrice):
            indice = riga * larghezza_matrice + colonna
            if indice < len(testo_pulito):
                carattere = testo_pulito[indice]
                
                # Assegna classe in base al tipo di evidenziazione
                classe = ''
                if indice in indici_saltatoria and indice in indici_semantici:
                    classe = 'saltatory-semantic'
                elif indice in indici_saltatoria:
                    classe = 'saltatory'
                elif indice in indici_semantici:
                    classe = 'semantic'
                
                html_matrice += f'<td class="{classe}">{html.escape(carattere)}</td>'
            else:
                html_matrice += '<td></td>'
